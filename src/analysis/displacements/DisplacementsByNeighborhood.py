...
class DisplacementByNeighborhood:
    spark = SparkSession \
        .builder \
        .appName("max-trips") \
        .getOrCreate()
    ...
    green_data_income_path = os.path.join(os.path.dirname(__file__), "output-data/green-income-by-area-displacements")
    green_data_outcome_path = os.path.join(os.path.dirname(__file__), "output-data/green-outcome-by-area-displacements")
    fhv_data_income_path = os.path.join(os.path.dirname(__file__), "output-data/fhv-income-by-area-displacements")
    ...
    def compute_yellow(self):
        ...
        income, outcome = self.count_displacements_by_area(yellow_df, self.area_id)
    ...
    def compute_green(self):
        ...
        income, outcome = self.count_displacements_by_area(green_df, self.area_id)
    ...
    def compute_fhv(self):
        ...
        income, outcome = self.count_displacements_by_area(fhv_df, self.area_id)
    ...
    def count_displacements_by_area(self, taxi_df: DataFrame, area_id):
        incomes = taxi_df.filter((col("PULocationID") == area_id) &
                                 (col("DOLocationID") < 264) &
                                 (col("DOLocationID") != col("PULocationID")) &
                                 (col("DOLocationID") != "")).groupBy("PULocationID","DoLocationID").count().orderBy(desc("count"))
        outcomes = taxi_df.filter((col("DOLocationID") == area_id) &
                                 (col("PULocationID") < 264) &
                                 (col("DOLocationID") != col("PULocationID")) &
                                 (col("PULocationID") != "")).groupBy("PULocationID", "DOLocationID").count().orderBy(desc("count"))
...
total_dis = DisplacementByNeighborhood(reader)
