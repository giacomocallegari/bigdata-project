...
class PassengersHours:
    spark = SparkSession \
        .builder \
        .appName("max-passengers-hours") \
        .getOrCreate()

    yellow_data_path = os.path.join(os.path.dirname(__file__), "output-data/yellow-passengers-hours")
    green_data_path = os.path.join(os.path.dirname(__file__), "output-data/green-passengers-hours")
    ...
    def compute_yellow(self):
        ...
        passengers_hour_df = Passengers.passengers_per_hour(yellow_df, fields)
        max_passengers = Passengers.max_passengers(passengers_hour_df, fields)
        if os.path.isdir(self.yellow_data_path):
            shutil.rmtree(self.yellow_data_path)
        passengers_hour_df.write.csv(self.yellow_data_path, header=False)
        self.yellow_chart = MapChart.MapChart(self.dbf_file, self.shp_file, self.yellow_data_path, 0, max_passengers, "Yellow Taxi Passengers - Hour of the day")
    ...
    def compute_green(self):
        ...
        passengers_hour_df = Passengers.passengers_per_hour(green_df, fields)
        max_passengers = Passengers.max_passengers(passengers_hour_df, fields)
        if os.path.isdir(self.green_data_path):
            shutil.rmtree(self.green_data_path)
        passengers_hour_df.write.csv(self.green_data_path, header=False)
        self.green_chart = MapChart.MapChart(self.dbf_file, self.shp_file, self.green_data_path, 0, max_passengers, "Green Taxi Passengers - Hour of the day")


reader = DataReader.DataReader()  # Initialize the DataReader
reader.read_input_params()  # Read the input parameters
aa = PassengersHours(reader)  # Set the reader
