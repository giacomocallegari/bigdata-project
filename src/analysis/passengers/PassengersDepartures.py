...
class Passengers:
    ...
    @staticmethod
    # Finds the passenger amount per pick-up location.
    def passengers_per_pickup_area(taxi_df: DataFrame) -> DataFrame:
        return taxi_df.groupBy(pu_loc).agg(avg(passengers)).orderBy('avg(' + passengers + ')', ascending=False)
    ...
    def compute_yellow(self):
        ...
        passengers_pu_area_df = self.passengers_per_pickup_area(yellow_df)
        max_passenger = self.__max_passengers(passengers_pu_area_df)
    ...
    def compute_green(self):
        ...
        passengers_pu_area_df = self.passengers_per_pickup_area(green_df)
        max_passenger = self.__max_passengers(passengers_pu_area_df)
