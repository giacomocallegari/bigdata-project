...
class PassengersMonths:
    spark = SparkSession \
        .builder \
        .appName("max-passengers-months") \
        .getOrCreate()

    yellow_data_path = os.path.join(os.path.dirname(__file__), "output-data/yellow-passengers-months")
    green_data_path = os.path.join(os.path.dirname(__file__), "output-data/green-passengers-months")
    ...
    def compute_yellow(self):
        ...
        passengers_month_df = Passengers.passengers_per_month(yellow_df, fields)
        max_passengers = Passengers.max_passengers(passengers_month_df, fields)
        if os.path.isdir(self.yellow_data_path):
            shutil.rmtree(self.yellow_data_path)
        passengers_month_df.write.csv(self.yellow_data_path, header=False)
        self.yellow_chart = MapChart.MapChart(self.dbf_file, self.shp_file, self.yellow_data_path, 0, max_passengers, "Yellow Taxi Passengers - Month of the month")
    ...
    def compute_green(self):
        ...
        passengers_month_df = Passengers.passengers_per_month(green_df, fields)
        max_passengers = Passengers.max_passengers(passengers_month_df, fields)
        if os.path.isdir(self.green_data_path):
            shutil.rmtree(self.green_data_path)
        passengers_month_df.write.csv(self.green_data_path, header=False)
        self.green_chart = MapChart.MapChart(self.dbf_file, self.shp_file, self.green_data_path, 0, max_passengers, "Green Taxi Passengers - Month of the month")


reader = DataReader.DataReader()  # Initialize the DataReader
reader.read_input_params()  # Read the input parameters
aa = PassengersMonths(reader)  # Set the reader
